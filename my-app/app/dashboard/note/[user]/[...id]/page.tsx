import NotePage from "@/components/note-page"
import { Note } from "@/lib/types"
import { cookies } from "next/headers"

const getNoteByID = async (user: string, id: string): Promise<{ status: boolean, note: Note | null }> => {
    const token = cookies().get("token")?.value

    const res = await fetch(`http://127.0.0.1:8000/api/v1/note/${user}/getnote/${id}`, {
        headers: {
            "Auth-Token": token as string
        },
        cache:"force-cache"
    })
    const data = await res.json()
    if (!res.ok) return { status: false, note: null }
    return { status: true, note: data?.data }
}


const Page = async ({ params }: { params: { user: string, id: string } }) => {

    const { note, status } = await getNoteByID(params?.user, params?.id)

    return (
        <div className='w-full min-h-screen p-4 flex flex-col items-center justify-center'>

            <NotePage data={note} status={status} />
        </div>
    )
}

export default Page